...
export default function ShippingAutoCalculator() {
  ...
  useEffect(() => {
    supabaseBrowser
      ...
      .eq("active", true)
      .order("sort_order")
      .then(({ data }) => setCards((data ?? []) as ShippingRateCard[]));
  }, []);
  ...
  const applyToSaleForm = () => {
    ...
    if (!input) {
      setStatus("売上登録の送料欄が見つかりません。売上登録画面を開いてください。");
      return;
    }
  ...
  return (
    <>
      <button
        ...
        style={{ position: "fixed", left: "50%", bottom: 280, transform: "translateX(-50%)", zIndex: 1100, border: 0, borderRadius: 999, padding: "13px 22px", background: "#0369a1", color: "white", fontWeight: 900, fontSize: 15, boxShadow: "0 8px 24px rgba(3,105,161,.25)" }}
      >
        🚚 送料自動計算
      </button>
        ...
        <div style={{ position: "fixed", inset: 0, zIndex: 1200, background: "rgba(15,23,42,.48)", display: "flex", alignItems: "center", justifyContent: "center", padding: 18 }}>
          <div style={{ width: "min(560px,100%)", maxHeight: "90vh", overflowY: "auto", background: "white", borderRadius: 24, padding: 24, boxShadow: "0 24px 80px rgba(0,0,0,.25)" }}>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              ...
              <button onClick={() => setOpen(false)} style={{ border: 0, background: "#f1f5f9", borderRadius: 999, padding: "8px 13px", fontWeight: 800 }}>閉じる</button>
            </div>

            <p style={{ color: "#64748b", lineHeight: 1.6 }}>配送先・配送会社・発送方法を選ぶだけで送料を算出し、売上登録の送料欄へ反映します。</p>
            ...
            <div style={{ display: "grid", gap: 14 }}>
              ...
              <label>
                送料を手動変更（任意）
                <input inputMode="numeric" value={manualAmount} onChange={(e) => setManualAmount(e.target.value.replace(/\D/g, ""))} placeholder={`自動：¥${amount.toLocaleString()}`} style={inputStyle} />
              </label>
            </div>
            ...
            <button onClick={applyToSaleForm} style={{ width: "100%", marginTop: 16, padding: 15, border: 0, borderRadius: 14, background: "#0369a1", color: "white", fontWeight: 900, fontSize: 16 }}>
              送料を売上登録へ反映
            </button>
