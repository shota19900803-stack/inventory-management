    ...
    <main style={{ minHeight: "100vh", background: "#f6f7f9", padding: "28px 18px 70px", color: "#111827", fontFamily: "system-ui, -apple-system, BlinkMacSystemFont, sans-serif" }}>
      <div style={{ maxWidth: 1100, margin: "0 auto" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 12, flexWrap: "wrap", marginBottom: 20 }}>
          ...
          <button onClick={() => router.push("/")} style={buttonStyle}>← 在庫管理へ</button>
        </div>
        ...
        <section style={cardStyle}>
          <label>商品を選択
            <select value={productId} onChange={(e) => setProductId(e.target.value)} style={inputStyle}>
              ...
              {products.map((item) => <option key={item.id} value={item.id}>{item.name}{item.jan_code ? ` / JAN ${item.jan_code}` : ""}</option>)}
            </select>
          </label>
        </section>
        ...
        {product && <>
          <section style={cardStyle}>
            ...
            <div style={{ display: "flex", gap: 18, flexWrap: "wrap", color: "#4b5563" }}><span>在庫：<strong>{Number(product.stock_quantity || 0)}個</strong></span><span>現在の参考仕入：<strong>{yen(Number(product.cost_price || 0))}</strong></span><span>参考販売：<strong>{yen(Number(product.selling_price || 0))}</strong></span></div>
          </section>
