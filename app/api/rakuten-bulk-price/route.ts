...
const EXCLUDED_WORDS = ["中古", "中古品", "ユーズド", "used", "ジャンク", "開封済み", "開封品", "箱なし", "箱無", "欠品", "訳あり", "アウトレット", "展示品", "リファービッシュ", "再生品", "部品", "パーツ", "難あり", "現状品"];
...
function extractModels(name: string) {
  const matches = name.match(/\b[A-Z]{1,6}-\d{2,}[A-Z0-9]*\b/gi) ?? [];
...
function coreName(name: string, model: string, brand: string) {
  ...
  for (const value of [model, brand, "BANDAI SPIRITS", "BANDAI", "MG", "HG", "RG", "PG", "1/100", "1/144", "色分け済み", "プラモデル", "ガンプラ"]) {
    if (value) s = s.replace(new RegExp(value.replace(/[.*+?^${}()|[\]\\]/g, "\\$&"), "ig"), " ");
  }
  s = s.replace(/\b\d{13}\b/g, " ").replace(/\s+/g, " ").trim();
  return s.split(" ").filter((x) => x.length >= 2).slice(0, 1).join(" ");
}
...
function buildQueries(p: Product) {
  ...
  const core = coreName(p.name, code, brand);
...
export async function POST(request: NextRequest) {
  ...
  for (const p of unique) {
    ...
    const debug: string[] = [];
    let completed = false;
    const queries = buildQueries(p).slice(0, 4);

    for (const query of queries) {
      try {
        const r = await ichibaSearch(appId, accessKey, query);
        const list = itemsOf(r.data);
        debug.push(`${query.slice(0, 24)}:${r.response.status}/${list.length}`);
        if (!r.response.ok) { debug.push(apiError(r.data, r.response.status)); continue; }
        const found = chooseIchibaItem(list, p, query);
        if (!found) continue;
        results.push({ jan: p.jan, price: found.price, productName: found.item.itemName ?? p.name, itemUrl: found.item.itemUrl ?? null, shopName: found.item.shopName ?? null, source: "rakuten-ichiba-item-search", matchedBy: query === p.jan ? "JAN" : "商品特定検索", elapsedMs: Date.now() - started, error: null });
        completed = true;
        break;
      } catch (e: any) {
        debug.push(e?.name === "AbortError" ? `${query.slice(0, 24)}:timeout` : `${query.slice(0, 24)}:${e?.message || "error"}`);
      }
    }

    if (!completed) results.push({ jan: p.jan, price: null, elapsedMs: Date.now() - started, error: debug.join(" / ") || "楽天市場の新品候補が見つかりませんでした。" });
  }
