  ...
  for (const input of unique) {
    ...
    try {
      ...
      if (exact.response.ok && direct.value != null) {
        results.push({ jan: input.jan, price: direct.value, lowestPrice: direct.value, rakutenLowestPrice: direct.value, productName: resolvedName || null, itemUrl: resolved?.productUrlPC ?? resolved?.productUrlMobile ?? resolved?.searchUrl ?? null, priceNaviUrl: resolved?.productUrlPC ?? resolved?.productUrlMobile ?? resolved?.searchUrl ?? null, shopName: null, source: "Rakuten Product Search / JAN exact", matchedBy: "JAN", candidateCount: resolved?.usedExcludeSalesItemCount ?? 0, newListingCount: resolved?.usedExcludeSalesItemCount ?? null, elapsedMs: Date.now() - started, priceSource: direct.source, debug: diagnostics, error: null });
        continue;
      }
